...
![Spatial registration. (**A**) A graph aligned to a background image within the normalized reference frame, so that nodes and edges are positioned against an external spatial reference such as a microscopy image. (**B**) Node placement under the two coordinate mappings: plot-space mapping, $(i-1)/(n-1)$, stretches node indices corner-to-corner across $[0,1]$, while pixel-space mapping, $(i-\tfrac{1}{2})/n$, places them at pixel centers; the two agree mid-axis but drift apart toward both ends.\label{fig:figure1}](figure1.png){width="100%"}
