...
*RGraphSpace* addresses this by taking an `igraph` object and scaling it into a normalized coordinate space in which nodes, edges, and their associated elements are resolved together. With node and edge geometries synchronized at rendering time, edges are drawn with respect to the final rendered extent of the nodes they connect, and this correspondence is maintained as aesthetics are mapped through the *ggplot2* grammar.

This normalized space places the graph within a broader spatial context, aligned to external reference frames such as microscopy images. Such alignment requires consistent coordinate conventions between the graph and the image; a mismatch in axis orientation, for example a top-left versus bottom-left origin, leaves the nodes reflected relative to the image (\autoref{fig:figure1}A). Moreover, because each pixel cell occupies a finite square area, a half-pixel offset is needed to prevent node positions from drifting toward the borders (\autoref{fig:figure1}B). This discrepancy may seem negligible in most cases but can matter when nodes represent measurements anchored to specific spatial features. Reconciling these conventions lets the graph be rendered as a coherent spatial object rather than an isolated diagram.
