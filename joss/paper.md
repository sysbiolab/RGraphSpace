...
Node coordinates are normalized either to a unit square or to a background image, with both the source image and a render-ready copy stored in the object. This establishes a stable reference frame for registering graphs to external coordinate systems. To keep this registration precise, `normalizeGraphSpace()` maps node coordinates to pixel centers through an explicit half-pixel offset.
