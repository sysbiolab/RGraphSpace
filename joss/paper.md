...
The normalized space serves a second purpose: it gives the graph a common spatial reference, allowing it to be registered against external coordinate systems, placing the graph within a broader spatial context. For images, spatial registration means that node coordinates are aligned and locked at the pixel level (\autoref{fig:figure1}A). This requires consistent coordinate conventions between the graph and the image; a mismatch in axis orientation, for example a top-left versus bottom-left origin, can cause the nodes and the image to be misaligned. Moreover, because each pixel occupies a finite square area, registering to a full plot space rather than the pixel center introduces a sub-pixel offset, drifting apart toward the plot edges (\autoref{fig:figure1}B). Such offsets may seem negligible in most cases, but can matter where high-resolution alignment is required, as in tissue photomicrographs.

![Spatial registration. (**A**) A graph aligned to a background image within the normalized reference frame, so that nodes and edges are positioned against an external spatial reference such as a tissue image. (**B**) Node placement under the two normalization modes: plot-space mapping, $(i-1)/(n-1)$, stretches node indices corner-to-corner across $[0,1]$, while pixel-space mapping, $(i-\tfrac{1}{2})/n$, places them at pixel centers; the two agree mid-axis and drift apart toward both ends.\label{fig:figure1}](figure1.png){width="100%"}
