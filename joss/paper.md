...
![Spatial registration. (**A**) A graph aligned to a background image, with nodes and edges positioned against an external reference frame such as a microscopy image. (**B**) Two normalization schemes: *anchors at pixel centers* use $(i-\tfrac{1}{2})/n$ with a half-pixel offset; *anchors at image border* use $(i-1)/(n-1)$, stretching positions across the image space $[0,1]$. The two coincide mid-axis but drift apart toward the ends.\label{fig:figure1}](figure1.png){width="100%"}
