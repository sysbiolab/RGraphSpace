...
.validate.igraph <- function(g, layout) {
    if (!is(g, "igraph")) {
    ...
    if(!is.null(layout)){
        if(nrow(layout)!=vcount(g)){
            msg <- paste("'layout' does not have xy-coordinates",
                "for the same number of nodes in 'g'")
            stop(msg)
        } else {
    ...
    } else if (!.is_numericVector(X) || !.is_numericVector(Y)) {
        msg <- paste0("'x' and 'y' vertex attributes should be ",
            "numeric vectors.")
        stop(msg, call. = FALSE
        )
    }
    if (is.null(vertex)) {
        ...
        warning(msg, call. = FALSE)
        vertex <- paste0("n", seq_len(igraph::vcount(g)))
    }
    if (!.all_characterValues(vertex)) {
        msg <- paste0("'name' vertex attribute should be a ",
            "character vector.")
        stop(msg, call. = FALSE
        )
    }
