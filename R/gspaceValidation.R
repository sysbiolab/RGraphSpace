...
.validate.edges <- function(g){
  atts <- .get.default.eatt(igraph::is.directed(g))
  a_names <- names(atts)
...
.set.undirected <- function(g, edges){
  g <- igraph::as.undirected(g, edge.attr.comb="ignore")
  g <- igraph::delete_edges(g, seq_len(ecount(g)))
  e <- rbind(edges[, 1], edges[, 2])
  attr <- as.list(edges[,-c(1,2)])
  g <- igraph::add_edges(g, e, attr = attr)
  return(g)
}
