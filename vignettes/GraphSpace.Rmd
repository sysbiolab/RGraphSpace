...
abstract: "<p>*GraphSpace* is an R package that integrates *igraph* and *ggplot2* graphics within spatial maps. *GraphSpace* implements new geometric objects using *ggplot2* protypes, customized for representing large *igraph* objects in a normalized coordinate system.</p>"
...
```{r Toy igraph - 3, eval=TRUE, message=FALSE, out.width="60%"}
# Plot the 'gtoy1' using GraphSpace
plotGraphSpace(gtoy1, marks = TRUE)
```


# *GraphSpace* attributes

Next, we will demonstrate all vertex and edge attributes that can be passed to *GraphSpace* methods.
...
Given a list of edges, *GraphSpace* represents only one edge for each pair of connected vertices. If there are multiple edges connecting the same vertex pairs, it will display the line attributes of the first edge in the list.
...
... and now plot the updated *igraph* object with *GraphSpace*:
 
```{r A shortcut for GraphSpace, eval=TRUE, message=FALSE, out.width="60%"}
# Plot the updated 'gtoy1' using GraphSpace
...
* Sysbiolab Team. "GraphSpace: A lightweight package for representing large igraph objects in a normalized coordinate system." R package, 2023.
