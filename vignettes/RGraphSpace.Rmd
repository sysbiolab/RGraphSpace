...
A seemingly simple yet technically challenging aspect of network visualization is ensuring that edges terminate exactly at the node boundary, regardless of the node sizes. This becomes more complex when node size is mapped to aesthetics and transformed by a `scale_size()` function, which is only evaluated within the layer where it takes effect.

The `geom_graphspace()` is designed to handle these adjustments automatically by rendering nodes and edges within a single, unified layer for internal synchronization. Below, we construct a star-like network with varying node sizes to show how the geometries stay  synchronized even across a wide range of `scale_size()` values.
...
In the first example, we use the default attributes stored in the `GraphSpace` object. The arrow offsets fit perfectly when node sizes are predefined, as we can rely on `npc` (Normalized Parent Coordinates) units for all network elements. This ensures that no matter how the graphical space is rescaled or the window is resized, nodes, edges, and arrows remain proportional to the viewport. This behavior is especially useful when overlaying networks on top of reference images (such as photomicrography or medical scans), where nodes must stay locked to specific pixel positions regardless of the output resolution.
...
When we map node size to a variable (like `num_var`), *ggplot2* rescales these values into a target range (e.g., `c(2, 40)`). This provides all the standard advantages of the *ggplot2* ecosystem, such as flexible graphical scaling and automatically coordinated legends.

There is, however, a subtle trade-off to keep in mind. When node size is mapped via `aes()`, *ggplot2* treats it as a fixed physical dimension (usually in `mm`) to maintain consistency with the legends. This means the node size will stay locked to the legends and will no longer scale proportionally if plotting area is resized.

In the example below, `geom_graphspace()` handles the bulk of the edge adjustment,
with the `arrow_offset` parameter providing additional manual fine-tuning.
...
While `geom_graphspace()` is convenient for quick, coordinated plots, it has a limitation: the `mapping` argument is reserved for node aesthetics. To overcome the limitations, we can call `geom_edgespace()` and `geom_nodespace()` as independent layers. This allows for total flexibility, such as mapping different variables to edges and nodes simultaneously.

However, since these layers are now independent, they no longer "talk" to each other by default. If we change the node size scale, the edge layer won't know it needs to adjust its offsets to accommodate the new node boundaries. The solution is `inject_nodespace()`. This function acts as a post-processing synchronizer: it intercepts the calculated sizes from the node layer and "injects" them into the edge layer.

```{r Adjusting scales - 4, eval=TRUE, message=FALSE, out.width="80%"}
# We shuffle 'num_var' to demonstrate that edges still 
# find their specific boundaries
...
To provide a final note of customization, these "physical size" trade-offs only apply when node size is passed as an aesthetic mapping. If no `size` aesthetic is defined, *RGraphSpace* defaults to using `npc` units for all network elements. This allows you to choose between the legend-coordinated scaling of *ggplot2* or the viewport-relative stability of `npc` units, depending on specific needs.
