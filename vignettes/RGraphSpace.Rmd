...
# We shuffle 'num_var' to demonstrate that edges still find their specific boundaries
set.seed(234)
gs_star$num_var2 <- sample(gs_star$num_var)

# Execute the plot, calling 'inject_nodespace' last to capture the final layer states
ggplot(data = gs_star) + 
...
As an example, next we extract topographic features from the `volcano` matrix and map these features to graph nodes. The graph is then visualized over the image matrix.
...
Next, we build a spatial network of cities with *RGraphSpace* `geoms` plugged into *ggraph* and *sf* workflows.
...
... and now plot; all the options below produce the same visual output to demonstrate how the package handles different types of input data.
