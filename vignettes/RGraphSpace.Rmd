...
*RGraphSpace* is an R package that generates *ggplot2* graphics for *igraph* objects [@Nepusz2006], scaling nodes and edges to a unit space. The package implements new *ggplot2* geometric prototypes [@Wickham2016] that use a dual-anchor normalization approach to align node and edge layers, ensuring that edge endpoints remain pinned to node boundaries regardless of node size. This is particularly useful when graph elements must be spatially aligned with reference maps or images. For extended documentation and use cases, see the [online tutorials](https://sysbiolab.github.io/RGraphSpace/).
