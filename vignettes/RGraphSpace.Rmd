...
2. **`geom_nodespace()`**: Dedicated to rendering vertices. Inherits `GeomPoint` aesthetic mappings, optimized to scale nodes and inform the edge layer on node states. It is used with the `inject_nodespace()` function to adjust edge offsets between layers. 
