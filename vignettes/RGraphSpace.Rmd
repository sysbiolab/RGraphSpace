...
# Extract pixel coordinates from the 'volcano' matrix,
# representing features at the 0.85 quantile level
coords <- which(volcano == quantile(volcano, 0.85), arr.ind = TRUE)

# Make an igraph from pixel coordinates
gtoy2 <- igraph::make_empty_graph(n = nrow(coords))
igraph::V(gtoy2)$y <- coords[,1]
igraph::V(gtoy2)$x <- coords[,2]

# Create a GraphSpace object, including the raster image;
# The 'mar' parameter controls the margins between the graph 
# and the image boundaries
gs <- GraphSpace(gtoy2, image = as_colorraster(volcano), mar = 0.1)

# Plot 'gs' with the raster as background
...
The examples below illustrate how *RGraphSpace* integrates with *ggplot2* using graph attributes stored in the `GraphSpace` object. Node and edge attributes can either define final visual properties (used as identity values) or be mapped to aesthetics and rendered through standard *ggplot2* geoms as building blocks.

In the following example, we create a small modular graph containing variables of different types in order to demonstrate several plotting options.
...
In this example, `nodeColor` already contains the final colour values stored in the `GraphSpace` object. Therefore, the colours can be used directly by mapping them and applying `scale_fill_identity()`.

```{r Plot identity values, eval=TRUE, message=FALSE, out.width="70%"}
ggplot() + 
  geom_edgespace(colour = "grey", data = gs) +
  geom_nodespace(aes(fill = nodeColor), colour = "grey", data = gs) +
  scale_fill_identity() +
  theme_gspace_axes(theme = "th0")
```

## Mapping categorical variables

Node and edge attributes can also be mapped to aesthetics and then rendered using appropriate scales. In this example, the node categorical variable `node_group` is mapped to `fill`, while the edge numeric variable `edge_var` is mapped to `colour`.

```{r Map aesthetics to categorical variables, eval=TRUE, message=FALSE, out.width="70%"}
ggplot() + 
  geom_edgespace(aes(colour = edge_var), data = gs) +
  geom_nodespace(aes(fill = node_group), colour = "grey", data = gs) +
...
Numeric attributes can also be mapped to aesthetics and visualised using continuous colour scales. In this example, both node and edge attributes are numeric variables.

```{r Map aesthetics to numeric variables, eval=TRUE, message=FALSE, out.width="70%"}
# Map aesthetics to numeric variables
ggplot() + 
  geom_edgespace(aes(colour = edge_var), data = gs) +
  geom_nodespace(aes(fill = node_var), colour = "grey", data = gs) +
  scale_colour_continuous(palette = c("cyan","blue")) +
...
When multiple geoms use the same aesthetic (for example `colour`) but should be mapped to different variables with independent scales, the *ggnewscale* package can be used to introduce a new scale.
...
ggplot() + 
  ...
  geom_nodespace(aes(colour = node_var), data = gs) +
  scale_colour_continuous(palette = c("white","purple")) +
  theme_gspace_axes(theme = "th0") + 
  theme(legend.position = "right")
```

This approach allows node and edge aesthetics to be controlled independently while still being drawn within the same plot.
