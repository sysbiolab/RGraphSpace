---
title: "Using RGraphSpace with Spatial Transcriptomics Data"
...
This vignette demonstrates how *RGraphSpace* renders the graph and spatial components of spatial transcriptomics data within the *ggplot2* grammar. Using spatial data from the *SeuratData* package, we show how a graph can be registered to its spatial coordinates and its features mapped to *ggplot2* aesthetics through the `GraphSpace` interface.
...
With the `GraphSpace` object ready, we can reproduce a spatial feature plot of the kind familiar from *Seurat*, using standard *ggplot2* syntax. Here we map expression of the `Ttr` gene to the `colour` aesthetic and display the tissue image as a background reference.
...
# Coercing spatial data {#sp-coercion}
