...
*RGraphSpace* provides two interfaces for styling nodes and edges: graph attributes (*camelCase* names such as `nodeColor` and `edgeColor`) and *ggplot2* mappings via `aes()`. The two interfaces coexist without collision; see the [Why *camelCase* attribute names?](#camelCase) section for details.
...
# Why *camelCase* attribute names? {#camelCase}
