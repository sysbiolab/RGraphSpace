...
The practical consequence is that `V(g)$fill <- "red"` and `V(g)$nodeColor <- "red"` are **not** equivalent. The former is a regular user variable, available for data-driven mapping via `aes()`. The latter is a pre-defined identity value applied directly to the rendered nodes.
...
3 | Graph attribute | Identity values from the `GraphSpace` object. | `geom_nodespace()`
::: 
