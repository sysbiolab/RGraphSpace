...
This section will create a toy `igraph` from scratch to demonstrate the vertex and edge attributes that *RGraphSpace* parses automatically. This shows exactly what the package expects as input. We will use `igraph`'s `make_star()` function and then `V()` and `E()` to assign attributes.
