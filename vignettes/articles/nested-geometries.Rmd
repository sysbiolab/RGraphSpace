...
The following code selects the five cells with the highest expression of the *PIGR* gene, adds arbitrary directed edges connecting them, and renders the resulting subgraph as an additional layer on top of the original visualization.
...
This deliberately naive example illustrates one possible approach to downstream graph manipulation. The edges have no biological meaning and are included solely to demonstrate how a graph can be modified at this later stage of the pipeline (see [*GraphSpace accessors*](graphspace-accessors.html) for an overview of the available graph transformations).
